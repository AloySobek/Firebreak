...
/*   Updated: 2019/12/02 18:36:20 by Rustam           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void SwapChain::setupSwapChain()
{
	...
	for (int i = 0; i < surface.khrFormatsCount; ++i)
		if (surface.formats[i].format == VK_FORMAT_B8G8R8A8_UNORM and surface.formats[i].colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR)
		{
	...
	!found ? sPresentMode = VK_PRESENT_MODE_IMMEDIATE_KHR : false;
...
void SwapChain::setupSwapChainCreateInfo()
{
	uint32_t	indexArray[2];

	indexArray[0] = device.physicalDevice.graphicQueueFamilyIndex;
	indexArray[1] = device.physicalDevice.presentQueueFamilyIndex;
	imageCount = surface.capabilities.minImageCount + 1;
	if (surface.capabilities.maxImageCount > 0 and imageCount > surface.capabilities.maxImageCount)
	...
	sCreateInfo.pQueueFamilyIndices = indexArray;
