...
/*   Updated: 2019/12/03 20:59:36 by Rustam           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 	surface.init(pWindow, instance);

// 	device.retrievePhysicalDevices(instance());
...
// 	device.checkCompatibleWithSurface(surface);
// 	device.setupQueue(VK_QUEUE_GRAPHICS_BIT);

// 	surface.checkSwapChainSupporting(device);

// 	device.create();
// 	device.retrieveAllQueus();

// 	instance.swapChain.setupSwapChain(surface);
// 	instance.swapChain.setupSwapChainCreateInfo(surface);
...
// 	createInfo.pNext = nullptr;
// 	createInfo.flags = 0;
// 	createInfo.queueFamilyIndex = device.graphicQueueFamilyIndex;
...
// 	vkAcquireNextImageKHR(device.self, instance.swapChain.self, std::numeric_limits<uint64_t>::max(), imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex);
...
// 	if ((error = vkQueueSubmit(device.graphicQueueFamily, 1, &submitInfo, VK_NULL_HANDLE)) != VK_SUCCESS)
// 	{
// 		std::cout << "Code of error: " << error << std::endl;
...
// 	vkQueuePresentKHR(device.presentQueueFamily, &present);
// }
...
{
	if ((error = glfwCreateWindowSurface(instance(), pWindow, nullptr, &surface.self)))
	{
		std::cout << "Code of error: " << error << std::endl;
...
// {
// 	vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDeviceObj, surface.self, &capabilities);
// 	vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDeviceObj, surface.self, &khrFormatsCount, nullptr);
// 	pKHRFormats = (VkSurfaceFormatKHR *)malloc(sizeof(VkSurfaceFormatKHR) * khrFormatsCount);
// 	vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDeviceObj, surface.self, &khrFormatsCount, pKHRFormats);
// 	vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDeviceObj, surface.self, &khrPresentModesCount, nullptr);
// 	pKHRPresentModes = (VkPresentModeKHR *)malloc(sizeof(VkPresentModeKHR) * khrPresentModesCount);
// 	vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDeviceObj, surface.self, &khrPresentModesCount , pKHRPresentModes);
...
void Vulkan::vkQueryPhysicalDeviceQueue()
{
	...
	{
		vkGetPhysicalDeviceSurfaceSupportKHR(physicalDeviceObj, i, surface.self, &presentQueueFamilySupport);
